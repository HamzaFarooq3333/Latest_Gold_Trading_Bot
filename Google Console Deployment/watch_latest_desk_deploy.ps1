...
<#
  UNIFIED TRIGGER: Any push to Latest_Gold_Trading_Bot by Hamza OR Ali
  -> safety_gate -> apply local workspace -> GCP live desk redeploy.

  Source of truth: HamzaFarooq3333/Latest_Gold_Trading_Bot
  Runs on Hamza office PC (needs gh + SSH/gcloud). Ali PC never runs this.
#>
[CmdletBinding()]
param(
  [int]$PollSeconds = 30,
  [int]$WaitAfterDetectSeconds = 12,
  ...
  [string[]]$AllowedLogins = @(
    "HamzaFarooq3333", "hamza", "Hamza",
    "alimacbookpro-web", "alimacbookpro", "ali", "Ali"
  ),
  [switch]$Once,
...
if (-not $CloneDir) {
  $CloneDir = Join-Path $env:TEMP "onyxion-latest-desk-watch-clone"
}
$StateFile = Join-Path $env:TEMP "onyxion-latest-desk-deploy-state.json"
$StatusFile = Join-Path $Workspace "Google Console Deployment\logs\latest_desk_deploy_status.json"
$LogFile = Join-Path $Workspace "Google Console Deployment\logs\latest_desk_deploy.log"
...
function Write-Status($obj) {
  try { ($obj | ConvertTo-Json -Depth 8) | Set-Content $StatusFile -Encoding UTF8 } catch { }
}
...
function Load-State {
  if (Test-Path $StateFile) {
    try {
      ...
      if ($s -and ($s.PSObject.Properties.Name -contains "last_sha")) {
...
function Save-State($state) {
  ([pscustomobject]@{ last_sha = [string]$state.last_sha } | ConvertTo-Json -Depth 4) |
...
function Test-IsAllowedCommit($info) {
  ...
  foreach ($a in $AllowedLogins) {
    $al = $a.ToLower()
    if ($login -eq $al) { return $true }
    if ($name -like ("*" + $al + "*")) { return $true }
    if ($email -like ("*" + $al + "*")) { return $true }
  }
  if ($login -match "hamza|alimacbook|^ali$") { return $true }
  if ($name -match "hamza|(^|[^a-z])ali([^a-z]|$)") { return $true }
  return $false
}
...
function Apply-LocalFromClone([string]$repoUrl, [string]$sha) {
  ...
  if (-not (Invoke-LocalSafetyGate -checkRoot $CloneDir)) { return $false }
  ...
  foreach ($p in $pairs) {
    ...
    if (-not (Test-Path $src)) { Write-WatchLog ("skip missing " + $p.From); continue }
    $dst = $p.To
...
function Process-Push($info) {
  $short = $info.sha.Substring(0, 7)
  $firstMsg = (($info.msg -split "`n")[0])
  Write-WatchLog ("PUSH DETECTED sha=$short by $($info.login)/$($info.name) msg=$firstMsg")
  Write-Status @{
    event = "push_detected"
    sha = $info.sha
  ...
  if (-not (Apply-LocalFromClone -repoUrl $url -sha $info.sha)) {
    Write-Status @{
  ...
  $ok = Deploy-GcpDashboard
  Write-WatchLog ("desk auto-deploy complete gcp_ok=$ok sha=$short")
  Write-Status @{
    event = "desk_deployed"
    sha = $info.sha
    author = "$($info.login)/$($info.name)"
...
Write-WatchLog "Latest->GCP desk watcher start poll=${PollSeconds}s repo=$Repo (Hamza+Ali)"
$state = Load-State
$info = Get-LatestSha $Repo
if ($info) {
  Write-WatchLog ("baseline sha=$($info.sha.Substring(0,7)) author=$($info.login)/$($info.name) allowed=$(Test-IsAllowedCommit $info)")
...
do {
  try {
    ...
    if ($info -and $state.last_sha -ne $info.sha) {
      ...
      $state.last_sha = $info.sha
      Save-State $state
      if (Test-IsAllowedCommit $info) {
        Process-Push $info
      } else {
        Write-WatchLog ("ignoring author $($info.login)/$($info.name)")
      }
    }
  } catch {
