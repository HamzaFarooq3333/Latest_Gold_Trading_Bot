# Live Trading Rules — Ali desk, current as of 2026-09-20
...
| Series | Used for |
|--------|----------|
| **Heikin-Ashi OHLC** (the bridge builds it from the raw MT5 bar) | **every candle rule**: previous-body break, X-Trend clearance, SUPP wick break, fill level, stop test, trailing, ATR |
| **Raw broker OHLC** | record tables and the reconcile mark price only — no rule reads it (since 2026-09-20; before that fills, stops and ATR used the raw bar) |
| `hist = EMA(WilderRSI(raw close, 3), 5) − 50` | colour — real close on purpose, so it matches the OnyxionHistogram MQ5 indicator on the MT5 chart |
...
Classic mode (`ENTRY_EVERY_CANDLE=0`, not live) keeps the old rules: amber arms, one primary per colour run, SUPP when the **next candle's body or wick crosses the wick of the candle the last trade was taken on** (HA high for buys / HA low for sells — the HA high already contains the body top, so one comparison covers both), up to `MAX_SUPP`.

## 3. Stops

| Setting | Live value |
|---------|------------|
| Stop distance | **1.25 × Wilder ATR(14)** of HA M15 bars at entry (`TSL_ATR_MULT`), fixed for that ticket |
| Fallback | `TSL_TICKS=1111` × `TSL_TICK_SIZE=0.001` = $1.111 when the multiplier is 0 |
| Trail | after every closed candle each ticket's stop ratchets to `HA close ∓ distance`; never loosens |
...
* per-ticket trailing stop hit (`HA low ≤ SL` for longs / `HA high ≥ SL` for shorts). The broker also holds the same SL as a real stop order, so a raw wick through it closes the ticket broker-side and the bridge reconciles that on the next poll.
...
1. **A hard-coded histogram threshold in the bridge** coloured bars now match `.env` HIST_THRESH=10 — every tunable is now read live from the environment (`effective_*`), including `hist_color`.
